...
dirs = ["z:\\Serialy\\Deuce - Špína Manhattanu\\seria 01 - 1080p\\", "z:\\Serialy\\Narcos\\seria 01 - 1080p.BluRay.DTS5.1.x264\\", "g:\\JDownloader\\serial Zlocin v Polne\\", "g:\\JDownloader\\serial How Not to Live Your Life s03\\", "g:\\JDownloader\\serial How Not to Live Your Life s02\\", "g:\\JDownloader\\serial How Not to Live Your Life s01\\"]
ext = ['.wmv', '.avi', '.mp4', '.mkv']
append = '_HEVC'
...
def encode_video(ffmpeg: pathlib.Path, i_file: pathlib.Path, o_file: pathlib.Path):
    ...
    command_line_ffmpeg = f'"{ffmpeg}" -i "{str(i_file)}" -map V -map a -map s? -c:v libx265 -preset veryfast -crf 26 -c:a copy -c:s copy "{str(o_file)}"'     
...
def edit_video(ffmpeg: pathlib.Path, mkvpropedit:pathlib.Path, i_file: pathlib.Path, o_file: pathlib.Path):
    ...
    return { str(i_file): { 'returncode_ff': ff.returncode, 'returncode_ss': ss.returncode, 'ff_args': ff.args, 'ss_args': ss.args, 'run_time': f"{round(end - start, 2)} seconds" } }
...
if __name__ == "__main__":
    pool = mp.Pool(int(mp.cpu_count() / 2)  - 2)
    params = mp_parameters(dirs=dirs)
    #results = pool.starmap(print_params, params)
    results = pool.starmap(edit_video, params)
    pool.close()
